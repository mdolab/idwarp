...
if mode == "train":
    ...
    # Run each script
    os.system("%s -np 1 python solve_script.py >> %s_reg.ref 2>&1" % (mpiexec, module_name))

    # Run each script
    os.system("%s -np 4 python solve_script.py >> %s_par_reg.ref 2>&1" % (mpiexec, module_name))
    ...
    # Run each script
    os.system("%s -np 1 python solve_script.py  >> %s_reg 2>&1" % (mpiexec, module_name))

    # Run each script
    os.system("%s -np 4 python solve_script.py  >> %s_par_reg 2>&1" % (mpiexec, module_name))
