...
parser.add_argument(
    "-mode", default="compare", choices=["train", "compare"], help="Train generates reference file. Compare runs test"
)
parser.add_argument("-diff_cmd", default="xxdiff", help="Command to run for displaying diff. Default: xxdiff")
parser.add_argument(
    "-nodiff",
    action="store_true",
    help="Suppress\
...
parser.add_argument("-mpiexec", default="mpirun", help="Command to use for running mpi")
...
if mode == "train":
    ...
    os.system("%s -np 1 python solve_script_cs.py >> %s_reg.ref 2>&1" % (mpiexec, module_name))

    # Run script in parallel: Note that the reference values are actually the
    # *REAL* values not the CS ones.
    os.system("%s -np 4 python solve_script_cs.py >> %s_par_reg.ref 2>&1" % (mpiexec, module_name))
    ...
    # Run script
    os.system("%s -np 1 python solve_script_cs.py complex >> %s_reg 2>&1" % (mpiexec, module_name))

    # Run script
    os.system("%s -np 4 python solve_script_cs.py complex >> %s_par_reg 2>&1" % (mpiexec, module_name))
