...
! floor
  ...
  interface cpu_time
     module procedure cpu_time_c
  end interface

  interface random_number
     module procedure random_number_c
  end interface

  interface dgesv
     module procedure dgesv_c1
     module procedure dgesv_c2
  end interface
...
! return the CPU time 
  subroutine cpu_time_c(z)
    complex*16, intent(out) :: z
    real*8 ::time
    
    call cpu_time(time)
    z = cmplx(time,0)
  end subroutine cpu_time_c

! return a random number time 
  subroutine random_number_c(z)
    complex*16, intent(out) :: z(:)
    real*8 r(size(z))
    integer n 
    n = size(z)
    ...
    z = cmplx(r,0)
  end subroutine random_number_c

! return a derivative through a linear solve 
  subroutine dgesv_c1(n, nrhs, a, lda, ipiv, b, ldb, info)
    INTEGER     INFO, LDA, LDB, N, NRHS
    complex*16  A( LDA, lda ),B( LDB )
    INTEGER     IPIV( n )
    
    call DGESV_c(n, nrhs, a, lda, ipiv, b, ldb, info)

  end subroutine dgesv_c1

  subroutine dgesv_c2(n, nrhs, a, lda, ipiv, b, ldb, info)
    INTEGER     INFO, LDA, LDB, N, NRHS
    complex*16  A( LDA, lda ),B( LDB, nrhs )
    INTEGER     IPIV( n )
    
    call DGESV_c(n, nrhs, a, lda, ipiv, b, ldb, info)

  end subroutine dgesv_c2

  subroutine det_c(n,x,detA)
    integer ::n
    complex*16 , dimension(n,n) :: x
    complex*16 :: detA
    real*8 :: detAr
    
    call det1(n,real(x),detAr)
    
    detA = cmplx(detAr,0)
  end subroutine det_c
